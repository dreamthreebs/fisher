...
sig_pann_cvl=np.load('../calc/test_sig.npy')
...
cs=log_Cubic_interpolate(dm_mass_set, sig_pann_cvl[:,2])
plt.loglog(xs,cs(xs),color='blue',linestyle='-',linewidth=1)
cs=log_Cubic_interpolate(dm_mass_set, sig_pann_cvl[:,1])
plt.loglog(xs,cs(xs),color='yellow',linestyle='-',linewidth=1)
cs=log_Cubic_interpolate(dm_mass_set, sig_pann_cvl[:,0])
