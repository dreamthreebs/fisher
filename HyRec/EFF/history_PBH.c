...
// Settings for Extended PBH routine
#define Minimum_BH_Mass 2E13
#define Maximum_BH_Mass 1E18
...
double dE_dtdV, dE_dtdV_Delta, BH_Temperature, eff_blackhole, Pi, Pre, BH_Mass_Min, BH_Mass_Max;
double MF, FUN, FUN2, Step,LM,M,PRECISION, Sigma, Mc, M1, M2, y,tmp, PBH_Mass_Function,FBH;


PBH_Mass_Function=(param->pann)*1E9;
Sigma=param->dm_gamma;
FBH=param->dm_bh_frac;
Mc=param->bh_mass;

// For integration subroutine
PRECISION=Precision;
BH_Mass_Min=Minimum_BH_Mass;
BH_Mass_Max=Maximum_BH_Mass;
...
dE_dtdV_Delta = eff_blackhole*(5.626976744186047e+29/cube(Mc)*FBH*(param->omh2-param->obh2)*cube(1.+z));
...
else if ( 0.5 < PBH_Mass_Function && PBH_Mass_Function < 1.5)
{
/* ------------------------------------ Begin_Log_Normal ------------------------------------*/
  if (Sigma<1E-5)
  {
...
/* --------------------------------------------1 */
  for (M_Index=0;M_Index<PRECISION;M_Index++)
  {
    ...
    eff_blackhole = interp_eff_eff(blackhole, BH_Temperature, 1.+z, BH_ENERGY_AXIS_SIZE, BH_REDSHIFT_AXIS_SIZE);
    FUN2 = eff_blackhole*(5.626976744186047e+29/cube(M)*FBH*(param->omh2-param->obh2)*cube(1.+z));
...
{ /* -------------------------------- Begin_Critical_Collapse --------------------------------*/
  LM=log10(BH_Mass_Min);
  Step=log10(BH_Mass_Max/BH_Mass_Min)/(PRECISION-1);
