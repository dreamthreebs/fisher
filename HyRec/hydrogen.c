...
/*            - December 2014: Added DM annihilation                                             */  
/*************************************************************************************************/ 
...
- Added additional ionization and excitations due to additional energy injection.
  dE_dtdV is the rate of energy injection per sec per cm^3
***************************************************************************************************/

double rec_TLA_dxHIIdlna(double xe, double xHII, double nH, double H, double TM, double TR, double Fudge, 
                         double fsR, double meR, double ionization_dE_dtdV, double alpha_dE_dtdV) {

   double RLya, alphaB_TM, alphaB_TR, four_betaB, C, s, Dxe2, DalphaB;

   double ion, exc;
   ion = ionization_dE_dtdV /nH /EI;  /* dxe/dt due to additional energy injection */
   exc = alpha_dE_dtdV /nH /E21;                      /* total rate of excitation to the n=2 state */
...
December 2014: now this also allocates memory for the tables, for less clutter in the main program
**********************************************************************************************/
...
/***********************************************************************************************
Free the memory for rate tables. Written december 2014 
***********************************************************************************************/
void free_rates(HRATEEFF *rate_table){
...
INPUT TEMPERATURE ASSUMED TO BE ALREADY RESCALED FOR VALUES OF alpha_fs and me
- Modified December 2014: DM annihilation sometimes brings Tm slightly larger than Tr.
...
void interpolate_rates(double Alpha[2], double DAlpha[2], double Beta[2], double *R2p2s, double TR, double TM_TR, 
    ...
    /*Tm > Tr variables*/
    double TM_TR_fixer;
    double logAlpha_fixer[2];

    logTR = log(TR);

    /* Check if TM/TR is in the range tabulated */ /*Only checks min*/
    if (TM_TR < TM_TR_MIN) {
      fprintf(stderr, "Error: TM/TR = %f is out of range in interpolate_rates.\n", TM_TR);
      exit(1);
    }

    /* If TM/TR > 1 apply fixing routine
       TM_TR_fixer = interpolate at edge of table
       logAlpha_fixer = dependence of table at low redshift alpha = AlphaConstant/(TM/TR)^constant */
    if (TM_TR > TM_TR_MAX) {
      TM_TR_fixer = TM_TR_MAX;
      logAlpha_fixer[0] = 0.536*log(TM_TR_MAX/TM_TR);
      logAlpha_fixer[1] = 0.742*log(TM_TR_MAX/TM_TR);
    }
    else {
      TM_TR_fixer = TM_TR;
      logAlpha_fixer[0] = 0;
    ...
    iTM = (long)floor((TM_TR_fixer - TM_TR_MIN)/rate_table->DTM_TR);
    if (iTM < 1) iTM = 1;
    if (iTM > NTM-3) iTM = NTM-3;
    frac1 = (TM_TR_fixer - TM_TR_MIN)/rate_table->DTM_TR - iTM;
    ...
    /* effective recombination coefficient to each level */
       for (k = 0; k < 4; k++) {
           temp[k] = rate_table->logAlpha_tab[l][iTM-1+k][iTR-1]*coeff2[0]
                   ...
                   + (coeff2[0] + coeff2[1] + coeff2[2] + coeff2[3])*logAlpha_fixer[l]; 
       }
...
- Added explicit dependence on xHII, which is not necessarily equal to xe if Helium has not entirely recombined
December 2014:
- Added dependence on extra energy injection
************************************************************************************************/

double rec_HMLA_dxHIIdlna(double xe, double xHII, double nH, double H, double TM, double TR, 
                          HRATEEFF *rate_table, double fsR, double meR, double ionization_dE_dtdV, double alpha_dE_dtdV){
   ...
   double ion, exc;
   ion = ionization_dE_dtdV /nH /EI;
   exc = alpha_dE_dtdV /nH /E21;   
...
void populateTS_2photon(double Trr[2][2], double *Trv[2], double *Tvr[2], double *Tvv[3], 
                        ...
                        double Alpha[2], double DAlpha[2], double Beta[2], double fsR, double meR, double alpha_dE_dtdV) {
   ...
   double exc = alpha_dE_dtdV /nH /E21;
...
double rec_HMLA_2photon_dxHIIdlna(double xe, double xHII, double nH, double H, double TM, double TR, 
                                  ...
                                  double zstart, unsigned iz, double z, double fsR, double meR, double ionization_dE_dtdV, double alpha_dE_dtdV){
   ...
   double ion = ionization_dE_dtdV/ nH /EI;
   ...
   populateTS_2photon(Trr, Trv, Tvr, Tvv, sr, sv, Dtau, xe, xHII, TM, TR, nH, H, rate_table, 
                      twog, Dfplus, Dfplus_Ly, Alpha, DAlpha, Beta, fsR, meR, alpha_dE_dtdV);
...
double rec_dxHIIdlna(int model, double xe, double xHII, double nH, double H, double TM, double TR, 
                     ...
                     double **Dfnu_hist, double zstart, unsigned iz, double z, double fsR, double meR, double ionization_dE_dtdV, double alpha_dE_dtdV){
   
    if      (model == PEEBLES)  return rec_TLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, 1.00, fsR, meR, ionization_dE_dtdV, alpha_dE_dtdV);
    else if (model == RECFAST)  return rec_TLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, 1.14, fsR, meR, ionization_dE_dtdV, alpha_dE_dtdV);
    else if (model == EMLA2s2p) return rec_HMLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, rate_table, fsR, meR, ionization_dE_dtdV, alpha_dE_dtdV);
    else if (model == FULL)     return rec_HMLA_2photon_dxHIIdlna(xe, xHII, nH, H, TM, TR, rate_table, twog, Dfminus_hist,  
                                                                  Dfminus_Ly_hist, Dfnu_hist, zstart, iz, z, fsR, meR, ionization_dE_dtdV, alpha_dE_dtdV);
    else {
